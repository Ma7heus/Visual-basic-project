...
If CAIXA_CHAPA = "" Then
    MsgBox "    � necessario informar uma chapa!"
    GoTo fim
End If
    ...
    ult_linha_two = Sheets("BAIXADOS").Cells(1, 1).End(xlDown).Row + 1
        For linha_two = 3 To ult_linha_two
            If ABA.Cells(linha_two, 3).Value = chapa Then
                    ...
                    Mac = ""
                    APARELHO1 = ""
                    MsgBox " Este smartphone j� foi Baixado, n�o � mais possivel usar-lo!!! "
            End If
         Next
    End If
Next
...
GoTo fim
Mensagem:
MsgBox "A chapa informada n�o � v�lida e/ou n�o existe. Tente novamente."
fim:
...
If CAIXA_CHAPA = "" Then
    MsgBox "    � necess�rio informar uma chapa!"
    Exit Sub
ElseIf CAIXA_PROFISSIONAL = "" Then
    MsgBox "    � necess�rio infomar o nome do profissional!"
    Exit Sub
ElseIf MATRICULA = "" Then
    MsgBox "    � necess�rio informar a matricula do profissional!"
    Exit Sub
ElseIf FILIAL = "" Then
    MsgBox "    � necess�rio informar uma filial para vinculo!"
    Exit Sub
ElseIf EMAIL = "" Then
    MsgBox "    � necess�rio informar o e-mail do profissional!"
    Exit Sub
ElseIf SENHA = "" Then
    MsgBox "    � necess�rio informar a senha do e-mail do profissional!"
    Exit Sub
ElseIf APARELHO = "" Then
    MsgBox "    � necess�rio informar o modelo do Smartphone!"
   Exit Sub
ElseIf IMEI = "" Then
    MsgBox "    � necess�rio informar o IMEI do Smartphone!"
   Exit Sub
ElseIf Mac = "" Then
    MsgBox "    � necess�rio informar o MAC do Smartphone!"
   Exit Sub
...
' Codigo que adiciona na tabela de mudan�as para controle das despesas.

linha_tabeladespesa = Sheets("MUDAN�AS").Cells(1048476, 1).End(xlUp).Row + 1
    Sheets("MUDAN�AS").Cells(linha_tabeladespesa, 1).Value = CAIXA_PROFISSIONAL.Value
    Sheets("MUDAN�AS").Cells(linha_tabeladespesa, 2).Value = FILIAL.Value
    Sheets("MUDAN�AS").Cells(linha_tabeladespesa, 3).Value = CAIXA_CHAPA.Value
    Sheets("MUDAN�AS").Cells(linha_tabeladespesa, 5).Value = DATA.Value
    Sheets("MUDAN�AS").Cells(linha_tabeladespesa, 4).Value = APARELHO1.Value
...
MsgBox "    Existem inconsit�ncias nas infoma��es preenchidas."
...
MsgBox "    O nome informado n�o � valido e/ou n�o existe. Tente novamente."
...
End Sub
Private Function S�N�meros(l As IReturnInteger)
    Select Case l
        Case Asc("0") To Asc("9")
            S�N�meros = l
        Case Else
            S�N�meros = 0
            MsgBox "Favor inserir apenas n�meros!", vbExclamation, "CAMPO TIPO N�MERO"
    End Select
End Function
